...
class SlackSearch:
                        ...
                        if hasattr(content_item, 'text'):
                            ...
                            for msg in msgs:
                                if isinstance(msg, str):
                                    ...
                                    # Check if channel is public (MCP fallback - we need to verify channel type)
                                    channel_id = parsed.get('channel_id', '')
                                    if channel_id:
                                        # Channel IDs starting with 'C' are typically public channels
                                        # 'D' = DM, 'G' = private group/channel
    ...
    def _search_slack_api(self, query: str, limit: int = 50) -> List[dict]:
        ...
        page = 1
        total_fetched = 0
        max_pages = 5  # Limit to 5 pages (500 results max) to avoid long waits
        ...
        try:
            while total_fetched < limit and page <= max_pages:
                ...
                params = {
                    'query': query,
                    'count': min(100, limit - total_fetched),  # Max 100 per page
                    'page': page,
                    'sort': 'timestamp',
                    'sort_dir': 'desc'
                }
                ...
                if 'messages' not in data or 'matches' not in data['messages']:
                    break
                ...
                for match in matches:
                    ...
                    is_im = channel_info.get('is_im', False)  # Direct messages
                    is_mpim = channel_info.get('is_mpim', False)  # Multi-party DMs

                    # Skip DMs and group messages - keep both public and private channels
                    # (forum- channels can be private)
                    if is_im or is_mpim:
                        continue
                    ...
                    msg = {
                        ...
                        'is_public': True  # Only public channels make it here
                    }
                    ...
                    if msg['ts']:
                        try:
                            ...
                            msg['timestamp'] = dt.strftime("%B %-d, %Y")
                        except:
